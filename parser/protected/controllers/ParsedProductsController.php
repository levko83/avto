...
class ParsedProductsController extends Controller
{
	...
	public function actionUpdate($id)
	{
        ...
        $shopProducts=new SiteProducts('search');
		...
		if(isset($_POST['ParsedProducts']))
		{
			...
			if($model->save())
            {
                ...
                if((isset($model->product_id) and strlen($model->product_id)>0) and (isset($model->product_type) and ($model->product_type == 'shina' or $model->product_type == 'disk')))
                {
                    SiteProducts::checkPrice($model->product_id, $model->product_type, false);
                }
				$this->redirect(array('view','id'=>$model->id));
            }
		}
        if (isset($_GET['SiteProducts']))
        {
         $shopProducts->attributes=$_GET['SiteProducts'];
        }
    ...
    public function actionDeleteAll()
    {
        ...
        {
            SiteProducts::checkPrice(false, false, false);

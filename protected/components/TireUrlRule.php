...
class TiresUrlRule extends CExtendsBaseUrlRule{
    ...
    private function _getDrivesPattern(){
        ...
        $pattern = "^shina\/(?<id>\d+){1}\-(?<avto>[a-z0-9\-]+))";
...
//            if(isset($params["v8"]) and !empty($params["v8"])){ // профиль
//                $url .= "/sezon=".ShinsSeason::model()->findByPk($params["v8"])->translit;
//            }

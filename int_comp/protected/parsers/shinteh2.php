...
class shinteh2
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=46;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'shinteh2'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="7654fa978f2f3ba76b8848aac865c3a9")
        {
            $charge_auto=Yii::app()->settings->get('system','charge_shina');
            for ($r=8; $r <= $this->lastRow; $r++)
            {

                $com_prod_ident=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$r)->getValue());
                ...
                {
                    $attr=array('company_id'=>46,'com_prod_ident'=>$com_prod_ident);

...
class intershunam
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=30;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'intershunam'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        for ($i=0;$i<7;$i++)
        {
            if (strlen(trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$i)->getValue()))>1)
            {
                ...
                if ($t_identity=='50bf82754af4ed5b2a0ccb21415f076a')
                {
        ...
        if ($this->price_identity==true)
        {
            for ($r=8; $r <= $this->lastRow; $r++)
            {
                ...
                if ((strlen($com_prod_ident)>30)&&strlen($price_t)>1)
                {
                    $attr=array('company_id'=>30,'com_prod_ident'=>$com_prod_ident);

...
class askania
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=25;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'askania'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        $this->price_identity=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,11)->getValue());
        ...
        if ($this->price_identity=="ccaf15e5726fa7f0c648556ddff0140a")
        {
            for ($r=11; $r <= $this->lastRow; $r++)
            {
                $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$r)->getValue();
                $prod_name=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(4,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$r)->getValue();
                $price_t=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(9,$r)->getValue();
				$price=ceil($price_t);
                $was=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(10,$r)->getValue();
                $amount_temp=array();
                ...
                if ((strlen($prod_name)>18)&&(strlen($price_t)>1))
                {
                    $attr=array('company_id'=>25,'com_prod_ident'=>$com_prod_ident);
            ...
            for ($r=11; $r <= $this->lastRow; $r++)
            {
                ...
                //$final_price=ceil($final_price);
                $charge_auto=Yii::app()->settings->get('system','charge_shina');
                if ((strlen($prod_name)>18)&&(strlen($price)>1))
                {
                    $attr=array('company_id'=>25,'com_prod_ident'=>$com_prod_ident);
                    $DBmodel=ParsedProducts::model()->findByAttributes($attr);
                    if ($DBmodel)

...
class favorit
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=47;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'favorit'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="4bb2889f535d02d7a6eeac8d2fb7108a")
        {
            $charge_auto=Yii::app()->settings->get('system','charge_shina');
            for ($r=13; $r <= $this->lastRow; $r++)
            {

                $com_prod_ident=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$r)->getValue());
                $prod_name=$com_prod_ident;
                $price_t=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$r)->getValue();
                $price=ceil($price_t/1.05);
                ...
                {
                    $attr=array('company_id'=>47,'com_prod_ident'=>$com_prod_ident);

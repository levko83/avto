...
class bcmarket
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=4;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'bcmarket'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        for ($r=1; $r<6; $r++) {
            ...
            if (trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(6,$r)->getValue())=='Модель' && trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(7,$r)->getValue())=='Бренд')
                {
        ...
        if ($this->price_identity===true)
        {
            for ($r=3; $r <= $this->lastRow; $r++)
            {
                ...
                $com_prod_ident=$sheet->getCellByColumnAndRow(0,$r)->getValue();
                $prod_name=$sheet->getCellByColumnAndRow(1,$r)->getValue().'/'.$sheet->getCellByColumnAndRow(2,$r)->getValue().'/'.$sheet->getCellByColumnAndRow(3,$r)->getValue().' '.$sheet->getCellByColumnAndRow(7,$r)->getValue().' '.$sheet->getCellByColumnAndRow(6,$r)->getValue();
                $price_t=trim($sheet->getCellByColumnAndRow(10,$r)->getValue());
				$price=ceil($price_t);
                $was=trim($sheet->getCellByColumnAndRow(12,$r)->getValue());
                $amount_temp=array();
                ...
                if (strlen(trim($sheet->getCellByColumnAndRow(2,$r)->getValue()))==2 )
                {
                    $attr=array('company_id'=>4,'com_prod_ident'=>$com_prod_ident);

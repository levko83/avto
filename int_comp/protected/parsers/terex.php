...
class terex
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=43;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'terex'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="cf5cd3043aa073ff29eb0fb82d6d3f1a")
        {
            ...
            for ($r=12; $r <= $this->lastRow; $r++)
            {
                ...
                if (preg_match('/шины/',$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$r)->getValue()))
                {
                    $charge_auto=Yii::app()->settings->get('system','charge_shina');
                }
                ...
                {
                    $attr=array('company_id'=>43,'com_prod_ident'=>$com_prod_ident);

...
class dtw
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=5;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'dtw'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        $this->price_identity=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,6)->getValue());
        ...
        if ($this->price_identity=="DTW Харьков")
        {
            for ($r=17; $r <= $this->lastRow; $r++)
            {
                $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$r)->getValue();
                $prod_name=$com_prod_ident;
                $price_t=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(7,$r)->getValue();
				$price=ceil($price_t);
                $was=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$r)->getValue();
                //$amount_temp=array();
                //preg_match_all ('/(\d+)/',$was,$amount_temp);
                //$amount=$amount_temp[0][0];
                $amount=0;
                if (strpos($was,'дней')) {$amount=0;}
                else {$amount=$was=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$r)->getValue();}
                $money_flag=840;
                $final_price=$price_t*$currency_value;
                $final_price=ceil($final_price);
                $charge_auto=Yii::app()->settings->get('system','charge_shina');
                if (!(strlen($com_prod_ident)<25))
                {
                    $attr=array('company_id'=>5,'com_prod_ident'=>$com_prod_ident);
            ...
            //Читаем второй лист прайса
            $this->objPHPExcel->setActiveSheetIndex(1);
            $this->lastRow=$this->objPHPExcel->getActiveSheet()->getHighestRow();
            for ($r=28; $r <= $this->lastRow; $r++)
            {
                $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$r)->getValue();
                $prod_name=$com_prod_ident;
                $price_t=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(4,$r)->getValue();
				$price=ceil($price_t);
                $was=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(6,$r)->getValue();
                $amount_temp=array();
                preg_match_all ('/(\d+)/',$was,$amount_temp);
                $amount=$amount_temp[0][0];
                $money_flag=840;

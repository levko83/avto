...
class vianor
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=15;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'vianor'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        $this->price_identity=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,10)->getValue());
        $this->price_identity.=($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,10)->getValue());
        //var_dump($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,10)->getValue());
        ...
        if ($this->price_identity=="77945bf2c780098fdfb7a54f93d3cbe1")
        {
            for ($r=13; $r <= $this->lastRow; $r++)
            {
                $com_prod_ident_t=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$r)->getValue();
                $art_pos=stripos($com_prod_ident_t,'(шт.)');
                $com_prod_ident=substr($com_prod_ident_t,0,$art_pos);
                $prod_name=$com_prod_ident;
                $money_flag=null;
                $price=null;
                $final_price=null;
                $avail=false;
                if ($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(4,$r)->getValue())
                {
                ...
                elseif ($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$r)->getValue())
                {
                    $price_t=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(4,$r)->getValue();
					$price=ceil($price_t);
                    $money_flag=980;
                ...
                //$price=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$r)->getValue();

                $was=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$r)->getValue();
                if ($avail && !substr_count($was,'-'))
                {
                    $amount_temp=array();
                ...
                if (stristr($com_prod_ident,'Диск'))
                {
                    $charge_auto=Yii::app()->settings->get('system','charge_disk');
                }
                if ((strlen($com_prod_ident)>18)&&($was!=''))
                {
                    $attr=array('company_id'=>15,'com_prod_ident'=>$com_prod_ident);
                    ...
                    else
                    {
                        ...
                        $model->company_id=Yii::app()->settings->get('system','charge_shina');

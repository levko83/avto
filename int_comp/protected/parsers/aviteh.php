...
class aviteh
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=17;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'aviteh'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="cf5cd3043aa073ff29eb0fb82d6d3f1a")
        {
            for ($r=8; $r <= $this->lastRow; $r++)
            {
                ...
                $was2=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(4,$r)->getValue();
                preg_match_all ('/(\d+)/',$was2,$amount_temp2);
                $amount=$amount_temp[0][0]+$amount_temp2[0][0];
                $money_flag=980;
                $final_price=$price;
                //$final_price=ceil($final_price);
                $charge_auto=Yii::app()->settings->get('system','charge_shina');
                if ((strlen($com_prod_ident)>18)&&(strlen($price_t)>1)&&(strlen($amount)>0))
                {
                    $attr=array('company_id'=>17,'com_prod_ident'=>$com_prod_ident);

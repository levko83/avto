...
class zorat
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=16;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'zorat'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="5bbab94f447ce2dadb240d90077d40db")
        {
            for ($r=8; $r <= $this->lastRow; $r++)
            {
                $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$r)->getValue();
                $prod_name=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$r)->getValue();
                ...
                {
                    $attr=array('company_id'=>16,'com_prod_ident'=>$com_prod_ident);
                    ...
                    else
                    {
                        ...
                        $model->flag_upd=2;
                        $model->charge_auto=Yii::app()->settings->get('system','charge_disk');
                        $model->amount=$amount;

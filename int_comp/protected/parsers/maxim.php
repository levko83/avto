...
class maxim
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=35;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'maxim'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        //$this->price_identity=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,10)->getValue());
        ...
        if ($this->objPHPExcel)
        {
            for ($s_column=0;$s_column<2;$s_column++)
            {
                if ($s_column==0) $column=0;
                if ($s_column==1) $column=6;
                $charge_auto=Yii::app()->settings->get('system','charge_shina');
                for ($r=2; $r <= $this->lastRow; $r++)
                {

                    if (preg_match('/ДИСК/',$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0+$column,$r)->getValue()))
                    {
                        $charge_auto=Yii::app()->settings->get('system','charge_disk');
                    }
                    $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0+$column,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1+$column,$r)->getValue();
                    $prod_name=$com_prod_ident;
                    $price_t=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(4+$column,$r)->getValue();
                    $price=ceil($price_t);
                    $was=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2+$column,$r)->getValue();
                    $amount_temp=array();
                    ...
                    {
                        $attr=array('company_id'=>35,'com_prod_ident'=>$com_prod_ident);

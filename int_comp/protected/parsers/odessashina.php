...
class odessashina
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=8;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'odessashina'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        $this->price_identity=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,5)->getValue());
        ...
        if ($this->price_identity=="www.odessashina.com")
        {
            for ($r=8; $r <= $this->lastRow; $r++)
            {
                $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$r)->getValue().'/R'.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(8,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(9,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$r)->getValue().$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(4,$r)->getValue();
                $prod_name=$com_prod_ident;
                $price_t=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(12,$r)->getValue();
                $price=ceil($price_t);
                $was=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(11,$r)->getValue();
                $amount_temp=array();
                ...
                {
                    $attr=array('company_id'=>8,'com_prod_ident'=>$com_prod_ident);

...
class parabashev
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=52;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'parabashev'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        for ($i=2;$i<6;$i++)
        {
            if (strlen(trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$i)->getValue()))>1)
            {
                ...
                //var_dump($t_identity);
                if ($t_identity=='ed504b3d4833375a1f19c82bc37fc0e8')
                {
        ...
        if ($this->price_identity==true)
        {
            for ($r=7; $r <= $this->lastRow; $r++)
            {
                ...
                $com_prod_ident_t=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$r)->getValue());
                $art_pos=stripos($com_prod_ident_t,'Арт.');
                ...
                {
                    $attr=array('company_id'=>52,'com_prod_ident'=>$com_prod_ident);

...
class koleso
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=32;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'koleso'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        for ($i=9;$i<16;$i++)
        {
            if (strlen(trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$i)->getValue()))>1)
            {
                ...
                if ($t_identity=='4f9163d8bb92dd9d8de16b367c59d717')
                {
        ...
        if ($this->price_identity==true)
        {
            for ($r=15; $r <= $this->lastRow; $r++)
            {
                ...
                {
                    $attr=array('company_id'=>32,'com_prod_ident'=>$com_prod_ident);

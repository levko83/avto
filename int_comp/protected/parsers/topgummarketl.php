...
class topgummarketl
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=13;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'topgummarketl'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        $this->price_identity.=($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,1)->getValue());
        ...
        if ($this->price_identity=="7e7facdac11b9096e9b67f317d654b2b")
        {
            for ($r=1; $r <= $this->lastRow; $r++)
            {
                $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(4,$r)->getValue();
                $prod_name=$com_prod_ident;
                $price_t=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(7,$r)->getValue();
				$price=ceil($price_t);
                $was=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(8,$r)->getValue();
                $amount_temp=array();
                ...
                {
                    $attr=array('company_id'=>13,'com_prod_ident'=>$com_prod_ident);

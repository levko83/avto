...
class replaywheeld
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=11;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'replaywheeld'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        $this->price_identity.=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,8)->getValue());
        ...
        if ($this->price_identity=="fdbb87a203bf9d17a41e3b1ed07ad340")
        {
            for ($r=9; $r <= $this->lastRow; $r++)
            {
                ...
                if ((strlen($prod_name)>18)&&strlen($price)>2)
                {
                    $attr=array('company_id'=>11,'com_prod_ident'=>$com_prod_ident);

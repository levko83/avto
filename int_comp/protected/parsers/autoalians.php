...
class autoalians
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=18;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'autoalians'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="72aa3f75fd330be91dcc5b9713db825c")
        {
            for ($r=14; $r <= $this->lastRow; $r++)
            {
                $com_prod_ident=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$r)->getValue());
                $prod_name=$com_prod_ident;
                $price_t=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$r)->getValue());
				$price=ceil($price_t);
                ...
                if ((strlen($com_prod_ident)>18)&& strlen($price_t)>2)
                {
                    $attr=array('company_id'=>18,'com_prod_ident'=>$com_prod_ident);

...
class kraina
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=34;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'kraina'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        $this->price_identity=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,11)->getValue());
        $this->price_identity.=($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,11)->getValue());
        ...
        if ($this->price_identity=="a9a5e1436b7d525d27dbe630aec04a01")
        {
            $charge_auto=Yii::app()->settings->get('system','charge_shina');
            for ($r=11; $r <= $this->lastRow; $r++)
            {

                if (strlen($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(3,$r)->getValue())<2)
                {
                ...
                {
                    $attr=array('company_id'=>34,'com_prod_ident'=>$com_prod_ident);

...
class goverlash
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=27;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'goverlash'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        $this->price_identity=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,2)->getValue());
        ...
        if ($this->price_identity=="2735a94899de780a0d0fa4237544131c")
        {
            for ($r=6; $r <= $this->lastRow; $r++)
            {
                $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(2,$r)->getValue();
                ...
                {
                    $attr=array('company_id'=>27,'com_prod_ident'=>$com_prod_ident);

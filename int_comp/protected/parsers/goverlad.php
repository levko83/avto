...
class goverlad
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=28;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'goverlad'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="2735a94899de780a0d0fa4237544131c")
        {
            for ($r=6; $r <= $this->lastRow; $r++)
            {
                ...
                if (!(strlen($com_prod_ident)<24))
                {
                    $attr=array('company_id'=>28,'com_prod_ident'=>$com_prod_ident);

...
class autoban
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=19;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'autoban'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="www.sdk-avtoban.com")
        {
            for ($r=12; $r <= $this->lastRow; $r++)
            {
                ...
                if ((strlen($com_prod_ident)>14)&&(strlen($price_t)>1))
                {
                    $attr=array('company_id'=>19,'com_prod_ident'=>$com_prod_ident);
            ...
            for ($r=12; $r <= $this->lastRow; $r++)
            {
                ...
                $com_prod_ident=trim(substr(trim($com_prod_ident_t),21));
                $prod_name=$com_prod_ident;
                ...
                $amount_temp=array();
                preg_match_all ('/(\d+)/',$was,$amount_temp);
                $amount=4*$amount_temp[0][0];
                $money_flag=980;
                //$final_price=$price*$currency_value;
                $final_price=price;
                $charge_auto=Yii::app()->settings->get('system','charge_disk');
                if ((strlen($com_prod_ident)>14)&&(strlen($price)>1))
                {
                    $attr=array('company_id'=>19,'com_prod_ident'=>$com_prod_ident);
                    $DBmodel=ParsedProducts::model()->findByAttributes($attr);
                    if ($DBmodel)

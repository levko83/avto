...
class npc
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=37;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'npc'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
       ...
       // $this->price_identity.=($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,10)->getValue());
        ...
        if ($this->price_identity=="14ebf2312194180c9c3247ad800896d5")
        {
            for ($r=7; $r <= $this->lastRow; $r++)
{
                $charge_auto=Yii::app()->settings->get('system','charge_disk');
                $com_prod_ident_t=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$r)->getValue());
                ...
                {
                    $attr=array('company_id'=>37,'com_prod_ident'=>$com_prod_ident);

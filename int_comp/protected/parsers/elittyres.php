...
class elittyres
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=6;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'elittyres'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        $this->price_identity=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,12)->getValue());
        ...
        if ($this->price_identity=="Elit-Tyres.com.ua")
        {
            for ($r=25; $r <= $this->lastRow; $r++)
            {
                ...
                if (strlen($prod_name)>10 && strlen($price_t)>1)
                {
                    $attr=array('company_id'=>6,'com_prod_ident'=>$com_prod_ident);

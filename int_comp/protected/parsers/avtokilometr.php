...
class avtokilometr
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=22;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'avtokilometr'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        for ($i=0;$i<5;$i++)
        {
            if (strlen(trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$i)->getValue()))>1)
            {
                $t_identity=trim($this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(5,$i)->getValue());
                $t_identity=md5($t_identity);
                //var_dump($t_identity);
                if ($t_identity=='687270fb8573c55fc4ebc2be30a823d4')
                {
        ...
        if ($this->price_identity==true)
        {
            ...
            {
                $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$r)->getValue();
                ...
                {
                    $attr=array('company_id'=>22,'com_prod_ident'=>$com_prod_ident);

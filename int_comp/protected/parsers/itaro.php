...
class itaro
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=31;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'itaro'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="cf5cd3043aa073ff29eb0fb82d6d3f1a")
        {
            for ($r=11; $r <= $this->lastRow; $r++)
            {
                ...
                if ((strlen($com_prod_ident)>26)&&strlen($price_t)>1)
                {
                    $attr=array('company_id'=>31,'com_prod_ident'=>$com_prod_ident);

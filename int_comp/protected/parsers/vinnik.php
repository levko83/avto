...
class vinnik
{
    ...
    {
        self::$data=null;
        self::$data['company_id']=33;
        $this->filename=$_SERVER['DOCUMENT_ROOT'].DIRECTORY_SEPARATOR.'int_comp'.DIRECTORY_SEPARATOR.'protected'.DIRECTORY_SEPARATOR.'prices'.DIRECTORY_SEPARATOR.'vinnik'.DIRECTORY_SEPARATOR.$filename;
    ...
    private function startParse()
    {
        ...
        if ($this->price_identity=="049a92a69817875bc5192307a8c5c86f")
        {
            for ($r=11; $r <= $this->lastRow; $r++)
            {
                $charge_auto=Yii::app()->settings->get('system','charge_shina');
                if (preg_match('/Диск/',$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$r)->getValue()))
                {
                ...
                $com_prod_ident=$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(1,$r)->getValue().' '.$this->objPHPExcel->getActiveSheet()->getCellByColumnAndRow(0,$r)->getValue();
                ...
                {
                    $attr=array('company_id'=>33,'com_prod_ident'=>$com_prod_ident);
